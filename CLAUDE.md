...
- **New Notification types**: `request_broadcast_created`, `request_response_submitted`, `request_accepted`, `request_rejected` (plain `String` column — no migration). **UI**: `app/app/requests/page.tsx` (tabs `?tab=mine|incoming`). **Entry point (REQBCAST-1d)**: the **`/app/saved` Browse toggle Services tab** (Stores · Products · Services) — `/app/saved` imports and renders `RequestsPage` verbatim (one source of truth). `/app/requests` is **kept as a standalone deep-link route** (notification deep-links to `?tab=incoming` still work); both surfaces render the same component. The old `/app/discover` "Post a request" CTA was removed — discover is purely the store filter/map surface again. **i18n**: 32 slugs (category `ui-requests`) seeded by `prisma/seed-requests-ui.js`, plus `app-search-services-tab` (category `ui-prodsearch`, seeded by `prisma/seed-prodsearch-ui.js`). **Verification**: `scripts/test-reqbcast.ts` — 25/25 against the live server with real minted sessions (16 service + 9 errand).
