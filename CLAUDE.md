...
- **Simple variant (CHAKRA-UI-4b)**: `/chakra/landing/simple` — monochrome twin of the landing that KEEPS the silhouette SVG + scroll journey (same stages/camera/IO/rail/cards/glyph-taps) but drops all colour and every ambient animation (no stars/water/halo-breathe/spin/pulse; active glyph gets a static white ring). Scroll-responsive transitions stay — nothing moves at rest. Same data + same "View details →" flow into `/chakra/[key]`; reuses the existing `ui-chakra` slugs; the full journey page is untouched.
