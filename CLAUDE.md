...
**Client trigger**: `app/store/[id]/setup/page.tsx` fires a fire-and-forget POST to this endpoint the first time the user clicks "Upload menu image", guarded by a `warmupFiredRef`. Now inert — left in place to avoid an unnecessary client change.
...
- **A Fleet initiative's services are invisible on the generic customer Store page (`/store/[id]`) — `/fleet/[pageId]` is the only customer-facing surface that works (FLEET-ORDER-1 investigation)**. Root cause: `/fleet/[pageId]` is keyed by `Page.id`; the customer Store page is keyed by a *different* id, `Store.id`/`Store.slug` (a Store row auto-created behind every Fleet page, per `### Fleet Initiative Type`). If a customer ever lands on `/store/{storeId}` for that same Fleet venture — e.g. it surfaces unfiltered in `GET /api/store/all` since that query has no `pageType` filter, so it can appear in `/app/saved` Browse → Stores and `/app/discover` — they see an **empty store with no items to order**. The reason: `app/store/[id]/page.tsx` (`productBlocks = section.blocks.filter(b => b.serviceType !== "delivery")`, line ~243) and `app/store/[id]/section/[sectionId]/page.tsx` (same filter, ~line 1021) both deliberately hide every `serviceType: "delivery"` block from the customer view — Fleet's entire "Fleet Services" section is 100% delivery blocks, so it renders as `return null` (section vanishes) on the storefront and as zero products on the section deep-link. This filter is intentional for *normal* stores (delivery blocks there are internal courier-assignment blocks, never meant to be customer-orderable) but it collaterally hides Fleet's customer-facing services when reached via the wrong URL. **Not fixed** — options for a future pass: (a) filter Fleet-backed stores out of `/api/store/all` and any discovery surface (cleanest — Fleet has its own dedicated booking page), or (b) make `/store/[id]/page.tsx` redirect to `/fleet/[pageId]` when `store.pageType === "fleet"` (same precedent as the slug→cuid canonical redirect), or (c) teach the customer Store page to render delivery blocks with the Fleet booking flow inline. Until one of these lands, always link customers to `/fleet/[pageId]`, never to the Fleet's backing `/store/[id]`.
...
**KNOWN GAP — fleet services are invisible on the generic customer Store page (`/store/[id]`)**, see the dedicated footgun entry below. Do not assume `/store/[id]` and `/fleet/[pageId]` show the same thing for a fleet initiative — they currently don't.
