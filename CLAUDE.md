...
## CHAKRA-1: Chakra Landing (Self-layer dashboard)
Seven chakras lit 0–100 by an "openness" score (0.6 platform signal + 0.4 user 1–7 self-report), a scrollable card list with per-chakra tagged todos + deep-link + self-report slider, over a static score-lit silhouette. Unified Todo channel: every todo carries `chakra` + `source`. **Full design: `docs/modules/chakra.md`.** Key facts:
...
- **Scoring**: `lib/chakra/score.ts` `computeChakraScores(userId)` → `Record<ChakraKey, { score, platform, self, platformOnly }>`. Pure read, no stored state. Platform signals: root=energy physical+funds; sacral=friends+posts+chat; solar=todo completion+course mastery; heart=existence of helping/community_group pages; throat=public posts+publicly-shared initiatives; third_eye=ConsultMessage count; **crown=PARKED dormant placeholder**. Every signal floored at `DORMANT=8` — empty never reads as 0/"broken". Served by `GET /api/chakra`.
...
- **UI**: `app/chakra/landing/page.tsx` (client). 2D SVG primary, static (no per-scroll transforms) for low-end Android; `/chakra/three` 3D path stays optional. Deep-links: root→/earn, sacral→/society, solar→/self, heart→/app/initiatives, throat→/society, third_eye→/listen, crown→disabled. Dormant copy is "ready to awaken" (hard requirement). 10 i18n slugs (category `ui-chakra`) seeded by `prisma/seed-chakra-ui.js` (160 rows = 10 × 16 locales, English fallback).
