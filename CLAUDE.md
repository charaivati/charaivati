...
The inDrive/noticeboard primitive. A user posts a service request; nearby providers offering that category get a notification card, respond (optionally with a quoted price), the requester accepts ONE, and both settle DIRECTLY via the accepted provider's UPI VPA. v1 ships `kind='service'` only. **Full design: `docs/modules/requests.md`.**
...
- **New Notification types**: `request_broadcast_created`, `request_response_submitted`, `request_accepted`, `request_rejected` (plain `String` column — no migration). **UI**: `app/app/requests/page.tsx` (tabs `?tab=mine|incoming`), CTA on `/app/discover`. **i18n**: 32 slugs (category `ui-requests`) seeded by `prisma/seed-requests-ui.js`. **Verification**: `scripts/test-reqbcast.ts` — 16/16 against the live server with real minted sessions.
