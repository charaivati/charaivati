...
**`TECH_DEBT.md` at the repo root is canonical.** `docs/TECH_DEBT.md` is a stale/secondary fragment (two old entries, not maintained) — add new tech-debt entries to the root file only.
...
## EXECPLAN 1-7: Execution Plan Hub (plan = the site's router)
The execution plan (`AiGoal.executionPlan` JSON) is the center of the user journey: it tells the user the ONE next page to go to, covers what the goal needs (skills/funds/environment/social/support — health deliberately excluded, it's a gate above the plan, never a plan dimension), and both landing skins + both chatbots read it. Plan doc: `docs/plans/EXECUTION_PLAN_HUB.md`.

- **Capability registry is the routing truth** (`lib/site/capabilityRegistry.ts`) — now covers `self.health/skills/funds/environment/energy/network` (all `/self?tab=personal&panel=X`), `business.evaluate` (`/business/idea`), `business.plan` (`/business`), `earn.initiatives` (`/app/initiatives`), `earn.hub` (`/earn`), `listen.consult` (`/listen`), `chakra.landing`. `SectionInfo` gained optional `dimension` + `panel`. SelfCanvas reads `?panel=` on mount and auto-opens that partner panel — registry routes deep-link into an open block. Add new destinations HERE, never hardcode routes in plan UI.
- **`ExecutionPlan.requirements`** (EXECPLAN-2, optional — absent on older plans): `{ skills[{name,status:have|learn}], funds{estimate?,note,businessNeeded}|null, environment[], social[], support:'none'|'consider_listen', layer:'self'|'society'|'nation'|'earth' }`. Generated in the skeleton step (maxTokens 700), sanitized leniently by `sanitizeRequirements()` — invalid block → `undefined`, never a failed skeleton.
- **Plan progress is PERSISTED — do not reintroduce ephemeral checkmarks** (EXECPLAN-3): every generated task carries `id`/`done`/`todoId` stamped at tasks-step time (todos created BEFORE the plan JSON is saved). `ExecuteBlock` derives checkmarks from `task.done`, persists toggles via the existing goal PATCH, and mirrors completion onto the linked Todo (`PUT /api/self/todos/[id]`) — plan, daily tasks and chakra `todos` signal agree by construction. `currentPhaseIndex` is clamped to `phases.length-1` on read. All four `/api/goal-ai/*` routes are rate-limited (plan 30/h, interview 120/h per user).
- **Hub UI** (EXECPLAN-4): `GoalExecuteSection` renders a health-first gate banner above the plan when `energyScore < 7` (display-only, opens Health panel — NEVER blocks). `ExecuteBlock`'s `RequirementsStrip` renders skills chips, funds note (+ "Build the business case →" `/business/idea` when `businessNeeded`), environment+social notes, a gentle `/listen` card for `consider_listen`, a layer badge, and an energy pill. Panel links resolve via `onOpenPanel` (in-canvas) or registry `?panel=` deep-links (standalone).
- **Chakra parity + toggle** (EXECPLAN-5): `/chakra/[key]` shows a compact plan card for active goals with `ARCHETYPE_CHAKRA[archetype] === key`; the landing stage card shows the plan's next action instead of the generic remark when one is live; both landings have a 🪷/▦ toggle (localStorage `charaivati.landing`, navigation-only for now).
- **Chatbots see the plan** (EXECPLAN-6): `buildUserContext` local tier includes an "Active plan" line (phase, next action + resolved route, done/total, requirements digest) — `/api/chat` and `/api/listen` get it via the shared composer. **LOCAL TIER ONLY — never add plan detail to `buildCloudBlock`** (UCTX-1b doctrine). `ai-context/EXECUTION_PLAN.txt` (sections `doctrine`/`routing`/`predrive`) loads into `/api/chat`'s static zone; `/listen`'s site map picks up registry changes automatically via `buildSiteAwareness()`.
- **Pre-drive doors** (EXECPLAN-7): "Not sure yet? Talk it out →" links to `/listen` from GoalsExpanded and the chakra root stage. When a drive proposal is accepted (`charaivati:profile-updated` with drives but no goals), SelfTab shows a one-time "Drive set — shape your first goal?" nudge whose CTA fires `charaivati:open-goal-creation` → SelfCanvas opens goal creation.
- **Deferred**: EXECPLAN-8 (outer-layer goal badges beyond the `layer` field); auto-seeding `requirements.skills` into per-goal skill lists.

## GOAL-UNIFY-1: One goal-creation path
**`lib/goals/createGoalRecord.ts` is the ONLY place a goal comes into existence.** It creates the `AiGoal` row (execution-plan pipeline) AND mirrors a `GoalEntry` into `Profile.goals` JSON (per-goal skills, funds/environment blocks) — deduped by case-insensitive statement, mirror entry `id` = `AiGoal.id` (the stable link between the two stores). The mirror is non-blocking: AiGoal is the primary write.

- **Callers**: `POST /api/self/goals` (used by `GoalCreationFlow` — the one manual add-goal UI — and by `SelfTab.saveGoalsToAiTable` for onboarding statement goals) and `POST /api/self/profile-proposal` (chat/Listen goal accepts now also create the AiGoal, entering the plan pipeline; the mirror dedupes against the entry `applyProfileProposal` just appended).
- **Do NOT call `prisma.aiGoal.create` directly in new code** — route through `createGoalRecord` so both stores stay in sync.
- **Onboarding double-write is safe by design**: onboarding saves profile goals client-side first; the server mirror dedupes by statement, so no doubles either way the race resolves.
- **`blocks/GoalBlock.tsx` was DELETED** (GOAL-UNIFY-1) — it was an unused duplicate goal UI (0 imports). The live pieces are `blocks/DriveBlock.tsx` (onboarding/drive picker) and `app/(with-nav)/self/tabs/goal-creation/GoalCreationFlow.tsx` (the single add-goal flow, opened from GoalsExpanded "✦ Add goal"). Do not resurrect GoalBlock.

## LANDING-SWITCH-1: Chakra is the post-login landing (EXPERIMENT — may revert)
Flow: land → choose language (`/`) → **`/chakra/landing`** (philosophy skin). `/self` is now the **Dashboard**, reached via the "Go to Dashboard →" button (top right on both `/chakra/landing` and `/chakra/landing/simple`).

- **What changed**: `app/page.tsx` logged-in bounce and the login page's default `redirectTo` both point at `/chakra/landing` (explicit `?redirect=` targets are still honoured); `/chakra` was added to `middleware.ts` `PROTECTED_ROUTES` (its data is user-scoped). The old EXECPLAN-5 "▦ Canvas view" top-right link on the landing was replaced by the Go to Dashboard button; `/self`'s "🪷 Chakra view →" link remains as the way back.
- **NOT deleted — obsoleted as a first screen only**: the AI-assisted drive/goal onboarding (`DriveBlock` OnboardingBanner) and `GoalCreationFlow` still live on the Dashboard (`/self`) exactly as before. A first-time user now meets the chakra journey first and sets drives/goals when they open the Dashboard.
- **Revert path** (if the experiment fails): flip the two defaults back to `/self` (`app/page.tsx` + login page) and drop `/chakra` from `PROTECTED_ROUTES`. Nothing else depends on the switch.
...
- **Simple variant (CHAKRA-UI-4b)**: `/chakra/landing/simple` — quiet, **minimal-colour** twin of the landing that KEEPS the silhouette SVG + scroll journey (same stages/camera/IO/rail/cards/glyph-taps) but drops every ambient animation (no stars/water/halo-breathe/spin/pulse). Scroll-responsive transitions stay — nothing moves at rest. Colour is restrained: silhouette/spine/inactive glyphs stay white line-art; the chakra's `c.color` appears only on the ACTIVE glyph + card accents (score ring, signal bars, bija, active border, rail dot, slider, CTA border). Desktop card pulled toward centre (`lg:pr-[20vw]`). Same data + same "View details →" flow into `/chakra/[key]`; reuses the existing `ui-chakra` slugs; the full journey page is untouched.
