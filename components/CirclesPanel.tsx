...
import { Plus, X, UserPlus, Pencil, Trash2, Building2 } from "lucide-react";
...
type FollowedPage = {
  id: string;
  page: { id: string; title: string; description?: string | null; avatarUrl?: string | null };
};

type PageItem = { id: string; title: string; description?: string | null; avatarUrl?: string | null };
...
function AddMemberModal({ circle, friends, onAdd, onClose }: {
  circle: Circle;
  friends: Friend[];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-sm p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <input autoFocus type="text" placeholder="Search friends…" value={query}
        ...
        <div className="space-y-1 max-h-56 overflow-y-auto">
          ...
          {filtered.map((f) => (
            <button key={f.id} disabled={adding === f.id}
              onClick={async () => { setAdding(f.id); await onAdd(circle.id, f.id); setAdding(null); }}
...
function NewCircleModal({ onCreate, onClose }: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-sm p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input autoFocus type="text" placeholder="Circle name…" maxLength={40} value={label}
...
function CircleCard({ circle, onAddMember, onRemoveMember, onDelete, onRename }: {
  circle: Circle;
  ...
  async function saveLabel() {
    ...
    if (trimmed && trimmed !== circle.label) await onRename(circle.id, trimmed);
    else setDraftLabel(circle.label);
    setEditing(false);
  }
    ...
    <div className={`relative flex-shrink-0 w-44 rounded-2xl border ${c.border} ${c.bg} p-4 flex flex-col gap-3`}>
      <div className="flex items-start justify-between gap-1">
        <div className="flex items-center gap-1.5 min-w-0 flex-1">
          ...
          {editing ? (
            <input ref={inputRef} value={draftLabel} onChange={(e) => setDraftLabel(e.target.value)}
              onBlur={saveLabel}
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={() => setEditing(true)} className="text-gray-600 hover:text-gray-300 transition-colors" title="Rename">
            <Pencil className="w-3 h-3" />
          </button>
          {!circle.isDefault && (
            <button onClick={() => onDelete(circle.id)} className="text-gray-600 hover:text-rose-400 transition-colors" title="Delete">
      ...
      <div className="flex flex-col gap-2 flex-1">
        {circle.members.length === 0 && <p className="text-xs text-gray-600 italic">No members yet</p>}
        {circle.members.map((m) => (
          <div key={m.id} className="flex items-center gap-2 group">
            ...
            <button disabled={removing === m.userId}
              onClick={async () => { setRemoving(m.userId); await onRemoveMember(circle.id, m.userId); setRemoving(null); }}
      ...
      <button onClick={() => onAddMember(circle)}
...
function OrgCard({ title, pages, color = "indigo" }: {
  title: string;
  pages: PageItem[];
  color?: string;
}) {
  const borderColor = color === "indigo" ? "border-indigo-500/30" : color === "teal" ? "border-teal-500/30" : "border-blue-500/30";
  const bgColor     = color === "indigo" ? "bg-indigo-500/10"     : color === "teal" ? "bg-teal-500/10"     : "bg-blue-500/10";
  const dotColor    = color === "indigo" ? "bg-indigo-400"         : color === "teal" ? "bg-teal-400"         : "bg-blue-400";

  return (
    <div className={`flex-shrink-0 w-44 rounded-2xl border ${borderColor} ${bgColor} p-4 flex flex-col gap-3`}>
      <div className="flex items-center gap-1.5">
        <span className={`w-2 h-2 rounded-full ${dotColor} shrink-0`} />
        <span className="text-sm font-semibold text-white truncate">{title}</span>
      </div>
      <div className="flex flex-col gap-2 flex-1">
        {pages.length === 0 && <p className="text-xs text-gray-600 italic">No pages yet</p>}
        {pages.map((p) => (
          <div key={p.id} className="flex items-center gap-2">
            {p.avatarUrl
              ? <img src={p.avatarUrl} alt={p.title} className="w-5 h-5 rounded-md object-cover flex-shrink-0" />
              : <div className="w-5 h-5 rounded-md bg-gray-700 flex items-center justify-center flex-shrink-0">
...
export default function CirclesPanel() {
  ...
  const [follows,    setFollows]    = useState<FollowedPage[]>([]);
  ...
  const loadAll = useCallback(async () => {
    const [circlesRes, friendsRes, followsRes, myPagesRes] = await Promise.all([
    ...
    const [cd, fd, fod, mpd] = await Promise.all([circlesRes.json(), friendsRes.json(), followsRes.json(), myPagesRes.json()]);
    if (cd.ok)  setCircles(cd.circles);
    if (fd.ok)  setFriends(fd.friends);
    if (fod.ok) setFollows(fod.follows);
    if (mpd.ok) setMyPages(mpd.pages);
    setLoading(false);
  }, []);
  ...
  async function handleAddMember(circleId: string, userId: string) {
    const res  = await fetch(`/api/circles/${circleId}/members`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ userId }) });
    const data = await res.json();
    if (data.ok) {
      setCircles((prev) => prev.map((c) => c.id === circleId ? { ...c, members: [...c.members, data.member] } : c));
  ...
  async function handleRemoveMember(circleId: string, userId: string) {
    const res = await fetch(`/api/circles/${circleId}/members`, { method: "DELETE", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ userId }) });
    if (res.ok) setCircles((prev) => prev.map((c) => c.id === circleId ? { ...c, members: c.members.filter((m) => m.userId !== userId) } : c));
  }

  async function handleCreateCircle(label: string, color: string) {
    const res  = await fetch("/api/circles", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ label, color }) });
    const data = await res.json();
    if (data.ok) { setCircles((prev) => [...prev, data.circle]); setShowNew(false); }
  }
  ...
  async function handleRenameCircle(circleId: string, newLabel: string) {
    const res  = await fetch(`/api/circles/${circleId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ label: newLabel }) });
    const data = await res.json();
    if (data.ok) setCircles((prev) => prev.map((c) => (c.id === circleId ? data.circle : c)));
  }
  ...
  return (
    <>
      <div className="space-y-5 pt-1">
        ...
        {/* ── People ── */}
        <div>
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">People</p>
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-white/10">
            ...
            <button onClick={() => setShowNew(true)}
              className="flex-shrink-0 w-44 rounded-2xl border border-dashed border-gray-700 bg-transparent
                hover:bg-gray-800/40 transition-colors flex flex-col items-center justify-center gap-2 py-8
        ...
        {/* ── Organizations ── */}
        <div>
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Organizations</p>
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-white/10">
            {/* Your Pages */}
            <OrgCard title="Your Pages" pages={myPages} color="indigo" />

            {/* Family Pages — pages followed by family circle members (derived client-side) */}
            <OrgCard
              title="Family Pages"
              color="teal"
              pages={follows
                .filter(f => {
                  const familyCircle = circles.find(c => c.label.toLowerCase() === "family");
                  if (!familyCircle) return false;
                  const familyUserIds = new Set(familyCircle.members.map(m => m.userId));
                  // follows are the current user's; we show all followed pages here as a placeholder
                  // until per-user page-follow data is available
                  return familyUserIds.size > 0;
                })
                .map(f => f.page)}
            />
            ...
            <OrgCard
              ...
              pages={follows
                .filter(f => {
                  const friendsCircle = circles.find(c => c.label.toLowerCase() === "close friends");
                  if (!friendsCircle) return false;
                  return friendsCircle.members.length > 0;
                })
                .map(f => f.page)}
            />
      ...
      {addingTo && (
        <AddMemberModal circle={addingTo} friends={friends} onAdd={handleAddMember} onClose={() => setAddingTo(null)} />
      )}
      {showNew && (
        <NewCircleModal onCreate={handleCreateCircle} onClose={() => setShowNew(false)} />
      )}
    </>
  );
}
