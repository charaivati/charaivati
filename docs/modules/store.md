...
- **GPS fallback UX (STORELOC-1)**: "Use my current location" calls `getCurrentPosition({ timeout: 15000, maximumAge: 60000, enableHighAccuracy: false })` — network-based positioning, since the pin is draggable and high accuracy isn't required. On failure the three `GeolocationPositionError` codes map to distinct copy (1 permission denied, 2 unavailable, 3 timeout), all pointing the owner at the draggable pin as the real fallback. Rendered in neutral gray, not red/amber — this is an expected fallback path, not an error state. The button always reverts to its idle label after success or failure so the owner can retry.
