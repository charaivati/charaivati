...
### Part D2 — Block / unblock (ACTION-INTENT-6)

Adds a `UserBlock` model and a bilateral-effect block feature, mirroring Part D's unfriend feature at every layer.

1. **Model** — `UserBlock { id, blockerId, blockedId, createdAt }`, `@@unique([blockerId, blockedId])`, indexes on both ID fields, cascade-delete via `User.blocksMade` (`@relation("BlocksMade")`) and `User.blocksReceived` (`@relation("BlocksReceived")`). Migration `20260620000000_add_user_block`. Requires `(db as any).userBlock` until a full non-`--no-engine` `prisma generate` (server stopped).
2. **Routes** — `POST /api/users/block { targetUserId }`: auth via `getCurrentUser`, idempotent `upsert` on `UserBlock`, then **block-implies-unfriend** — deletes the `Friendship` row via `canonicalPair()` and clears any pending `FriendRequest` in either direction. `POST /api/users/unblock { targetUserId }`: deletes the `UserBlock` row only (does not restore the friendship — re-friending requires a new request, now possible again since the block is gone).
3. **Bilateral enforcement**:
   - `lib/users/searchUsers.ts` excludes a candidate `u` from results if EITHER side has blocked the other: `u.blocksReceived: { none: { blockerId: excludeUserId } }` AND `u.blocksMade: { none: { blockedId: excludeUserId } }`. Neither party can find the other in search, in either direction.
   - `POST /api/friends/request` and `POST /api/listen/actions/friend-request` check for a `UserBlock` in either direction before creating a `FriendRequest`; on a match return `{ ok: false, error: "blocked", message: "Couldn't send a friend request." }` (400). **Wording is deliberately neutral** — never reveals which side blocked, or that a block is the reason at all.
4. **Chat trigger** — `lib/ai/actionTrigger.ts` exports `BLOCK_TRIGGERS = ["block", "unblock"]` and `isBlockRequest(message)` — a broad substring match (see `TECH_DEBT.md` § 17 for the false-positive tradeoff and why it's accepted). Looser phrasings ("I don't want X to contact me anymore", "stop him from messaging me", "I don't want to hear from her again") are caught by the intent classifier's new `block_user` intent (`lib/listener/intentClassifier.ts`), which is explicitly prompted to disambiguate from `remove_friend`: "block_user is stronger than remove_friend — if the user wants to cut off ALL contact (not just unfriend), use block_user."
5. **Extraction** — `extractBlockQuery(text, model)` (`lib/listener/actions.ts`, one `chatComplete jsonMode` call) → `{ name: string | null }`.
6. **Deterministic action building** — `buildBlockAction(userId, name)` searches the user's `Friendship` rows first (matches unfriend's behavior for friends); if no friend matches, falls back to `searchUsers({ q: name, excludeUserId: userId, limit: SEARCH_MAX_RESULTS })` since **block can target a non-friend** (unlike unfriend). One match → `{ type: "block_confirm", target }`; multiple matches → `{ type: "block_pick", candidates }`; zero matches → `{ type: "block_not_found", name }`. `describeBlockReply(action)`: "Are you sure you want to block {name}? They won't be able to find you, friend you, or contact you, and any existing friendship will be removed." / pick-list prompt / "I couldn't find anyone by that name."
7. **Action types** (`lib/listener/actionTypes.ts`): `block_confirm`, `block_pick`, `block_not_found`.
8. **Shared card refactor** — `components/listen/FriendshipActionCard.tsx` is a new shared component extracted from the original `UnfriendCard`, implementing the `idle → sending → done/dismissed/error` confirm machine and styling once. Props: `{ friend: {id,name,avatarUrl}, verb: "Unfriend"|"Block", endpoint, confirmText?, sendingText?, successText? }`. POSTs `{ friendId, targetUserId, userId }` (all the same id) so either `/api/friends/remove` (reads `friendId`) or `/api/users/block` (reads `targetUserId`/`userId`) works unchanged. `UnfriendCard.tsx` (now a thin wrapper resolving `unfriend_pick`/`unfriend_confirm` to a target and rendering `FriendshipActionCard` with `verb="Unfriend"` and `endpoint="/api/friends/remove"`) and the new `BlockCard.tsx` (same shape, `verb="Block"`, `endpoint="/api/users/block"`) both render via this shared card. `unfriend_not_found`/`block_not_found` render nothing — the reply text covers it.
9. **Wiring in `app/api/listen/route.ts`** — mirrors the unfriend dispatch exactly: a strict-keyword branch (`!pendingHandled && isBlockRequest(text)` → `extractBlockQuery` → `buildBlockAction` → `describeBlockReply`), a classifier-fallback branch (`classified.intent === "block_user"` → same three calls), and `isBlockRequest(text)` added to the pendingReminder continuation's "another action takes precedence" OR-chain.
10. **Confirmation required** — block is destructive (auto-unfriends + cuts off contact) and is never auto-applied; both `block_confirm` and the post-pick confirm step require an explicit tap.
11. **Deferred** — unblock has no chat-confirm flow yet (`isBlockRequest` matches "unblock" but `buildBlockAction` only ever produces block-direction actions); `POST /api/users/unblock` exists and works but isn't reachable from chat. No "blocked users" list UI. See `TECH_DEBT.md` § 17.
...
BLOCK SOMEONE
- What it does: blocks another user — they won't be able to find you in search, send you a friend request, or contact you. If you're currently friends, blocking also removes that friendship.
- How it's asked: "block X", "block him", "I don't want X to contact me anymore", "stop X from messaging me", "X is harassing me, can you block them".
- Confirm step: yes, always — blocking is destructive (it also ends any existing friendship), so it's shown as a clear confirm/cancel before anything changes. Blocking does not need to be a current friend — it can target anyone.
