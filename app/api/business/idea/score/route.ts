...
function scoreResponse(question: Question, answer: string): number {
  // For multi-choice questions, extract score from option
  ...
  // For text questions, simple heuristic (can be improved)
  let score = 0;
  const answerLower = answer.toLowerCase();

  // Length-based scoring for text
  if (answer.length < 20) {
    score = -1;
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const [questionId, answer] of Object.entries(responses)) {
      ...
      // Save individual response with score
    ...
    // Update idea with final scores
    const updatedIdea = await prisma.businessIdea.update({
      where: { id: ideaId },
      data: {
        problemClarity: dimensionScores.problemClarity || 0,
        marketNeed: dimensionScores.marketNeed || 0,
        targetAudience: dimensionScores.targetAudience || 0,
        uniqueValue: dimensionScores.uniqueValue || 0,
        feasibility: dimensionScores.feasibility || 0,
        monetization: dimensionScores.monetization || 0,
        status: "submitted",
        responses,
      },
      include: {
    ...
    console.error("POST /api/business/idea/score", error);
    return NextResponse.json(
