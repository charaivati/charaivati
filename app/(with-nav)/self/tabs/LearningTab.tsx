...
export default function LearningTab() {
  ...
  function updateGoal(drive: DriveType, index: number, patch: Partial<GoalEntry>) {
    const current = [...(goalsByDrive[drive] ?? [])];
    current[index] = { ...defaultGoal(), ...current[index], ...patch };
    updateGoals(drive, current);
  }

  function removeGoal(drive: DriveType, index: number) {
    const current = [...(goalsByDrive[drive] ?? [])];
                ...
                <div key={drive} className="space-y-3">
                  ...
                  {goals.map((goal, index) => (
                    <GoalCard
                      ...
                      goal={goal}
                      index={index}
                      onUpdate={(patch) => updateGoal(drive, index, patch)}
                      onRemove={() => removeGoal(drive, index)}
                    />
                  ))}
